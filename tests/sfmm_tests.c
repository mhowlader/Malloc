...
Test(sfmm_student_suite, student_test_2, .timeout = TEST_TIMEOUT) {
	...
	double frag = ((double)20)/32;
    cr_assert(sf_internal_fragmentation()==frag,"Internal Fragmentation Value not Correct!");
    sf_free(r);
...
Test(sfmm_student_suite, student_test_3, .timeout = TEST_TIMEOUT) {
    ...
    sf_free(r);
    double frag = ((double)120)/1024;

	cr_assert(sf_peak_utilization()==frag,"Internal Fragmentation Value not Correct!");
...
Test(sfmm_student_suite, student_test_4, .timeout = TEST_TIMEOUT) {
	...
	//double frag = ((double)20)/32;
    sf_free(r);
	cr_assert(sf_internal_fragmentation()==0,"Internal Fragmentation Value not Correct!");


    //printf("%f\n",sf_internal_fragmentation());
}

//peak utilization with different size blocks
